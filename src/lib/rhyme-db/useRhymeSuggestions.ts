      ...
      if (preferLocal) {
        ...
        try {
          ...
          const mergeDebug = (items: Array<RhymeTokenDebug | undefined>): RhymeTokenDebug | undefined => {
            ...
            const mergeGateCounts = (key: 'afterGates' | 'commonAfterGates') => {
              const counts = available
                .map((item) => item[key])
                .filter((value): value is NonNullable<RhymeTokenDebug['afterGates']> => Boolean(value))
              if (counts.length === 0) return undefined
              return {
                near: Math.max(...counts.map((value) => value.near)),
                slant: Math.max(...counts.map((value) => value.slant)),
              }
            }
            ...
            return {
              ...
              candidatePools,
              afterGates: mergeGateCounts('afterGates'),
              commonAfterGates: mergeGateCounts('commonAfterGates'),
              thresholds: pickFirst('thresholds'),
              fallbackUsed: available.some((item) => item.fallbackUsed),
              topEntries: pickFirst('topEntries'),
            }
          }
